    ...
    <div className="space-y-12">
      <div className="max-w-2xl">
        <div className="relative">
          ...
          <Input 
            ...
            placeholder="Search properties by title, location, or keyword..." 
            className="pl-12 rounded-full border-neutral-800 bg-neutral-900/50"
          />
        </div>
      </div>
      ...
      {properties.length === 0 ? (
        ...
        <div className="space-y-8">
          <h2 className="text-[11px] font-bold uppercase tracking-[0.3em] text-neutral-600">Search Results</h2>
          {filteredProperties.length === 0 ? (
            <div className="text-neutral-500 py-12 text-[11px] uppercase tracking-widest">No properties matched your search.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredProperties.map((property) => (
                <Link key={property.id} to={`/property/${property.id}`} className="group block focus:outline-none space-y-4">
                  <div className="w-full aspect-[4/3] bg-neutral-900 border border-neutral-800 overflow-hidden relative flex items-center justify-center">
                    ...
                    <div className="absolute top-4 left-4 bg-black/80 backdrop-blur-sm text-white px-3 py-1 text-[10px] font-bold uppercase tracking-tighter z-10">
                      ${property.price.toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <h3 className="text-xl font-light tracking-tight group-hover:text-neutral-300 transition-colors truncate">{property.title}</h3>
                    <p className="text-[11px] uppercase tracking-widest text-neutral-500 truncate mt-1">{property.location}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16">
          <section className="col-span-1 md:col-span-7 flex flex-col">
            <Link to={`/property/${filteredProperties[0]?.id}`} className="group block focus:outline-none">
              <div className="w-full aspect-[4/3] bg-neutral-900 border border-neutral-800 relative mb-8 flex items-center justify-center overflow-hidden">
                <div className="absolute top-6 left-6 bg-white text-black px-3 py-1 text-[10px] font-bold uppercase tracking-tighter z-10">
                  Featured - {filteredProperties[0]?.listingType === 'buy' ? 'Wanted' : 'Sale'}
                </div>
                {filteredProperties[0]?.imageUrl && filteredProperties[0]?.imageUrl !== 'default' ? (
                  <img 
                    src={filteredProperties[0]?.imageUrl} 
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div className="space-y-2">
                  <h1 className="text-4xl font-light tracking-tight group-hover:text-neutral-300 transition-colors">{filteredProperties[0]?.title}</h1>
                  <p className="text-neutral-500 font-serif italic text-lg">{filteredProperties[0]?.location}</p>
                </div>
                <div className="sm:text-right shrink-0">
                  <div className="text-3xl font-light tracking-tighter">${filteredProperties[0]?.price.toLocaleString()}</div>
                  <button 
                    onClick={(e) => {
                      e.preventDefault();
          ...
          <section className="col-span-1 md:col-span-5 flex flex-col md:border-l border-neutral-900 md:pl-16 mt-12 md:mt-0">
            ...
            <div className="space-y-12">
              {filteredProperties.slice(1).map((property, idx) => (
                <motion.div 
                  key={property.id}
